   ...
   <div className="p-4">
      ...
      <div className="w-60 h-[95.5vh] bg-[#E7E9EB] p-4 flex flex-col justify-between rounded-4xl">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-[#073D44] text-[1.2rem] font-[productsans]">Tutor.AI</span>
        </div>
        {/* Top Menu */}
        <div className="mb-20">
          <h2 className="text-[#6E6E6E] text-sm font-semibold mb-4 font-[gilroy]">Menu</h2>
          <ul className="space-y-2">
            {menuItems.map((item) => (
              <li key={item.name}>
                <button
                  onClick={() => setActive(item.name)}
                  className={`flex items-center gap-3 w-full px-4 py-2 font-[gilroy]  text-sm font-medium transitio
                  ${active === item.name
          ...
          <h2 className="text-[#6E6E6E] text-sm font-semibold mt-6 mb-4 font-[gilroy]">General</h2>
          <ul className="space-y-2">
        ...
        <div className="mt-4 mb-7 flex justify-center">
          <button className="flex items-center gap-3 w-[65%] justify-center px-4 py-2 bg-[#FF4545] text-white rounded-[10px] text-sm font-medium hover:bg-red-600 transition">
            <LogOut size={18} />
            Log out
          </button>
        </div>
